...
// Last modified: 04.11.2021 by Eggert Mar Eggertsson (eggerte19@ru.is)
#include <stdio.h>
#include <errno.h>
#include <stdlib.h>
#include <unistd.h>
...
// source for solution: 
// https://www.tutorialspoint.com/How-to-execute-a-command-and-get-the-output-of-command-with// in-Cplusplus-using-POSIX
...
{
    if (send(clientSocket, message, strlen(message), 2048) < 0) // CHANGED 0 -> 1025, resolved segmentation fault issues
    {
                  ...
                  char *buffer) 
{
  ...
  if((tokens.size() >= 2) && (tokens[0].compare("SYS") == 0))
  {
    ...
    std::cout << "this is the command: " << command << "\n"; // Server outputs the client commands. 
    if ((response = exec(command.c_str())).empty()) {
